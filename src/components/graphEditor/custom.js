...
//import {fireEvent} from 'mxgraph/javascript/src/js/view/mxGraph'
...
export function dblClick(vm,evt, cell)
		{
                ...
                console.log('pt',pt);
                console.log('vm',vm);
                vm.graph.isEnabled = function() { return false; }
            if (vm.graph.isEnabled())
			{
				...
				if (evt != null && !vm.model.isVertex(cell))
				{
						...
						(MxClient.IS_SVG && src == vm.view.getCanvas().ownerSVGElement)))
					{
						...
						cell = this.addText(vm,pt.x, pt.y, state);
					}
                }
            }
       ...
       function addText(vm,x, y, state)
		{
			...
			{
				var tr = vueItem.view.translate;
				label.style = 'text;' + style;
				label.geometry.width = 40;
				label.geometry.height = 20;
				label.geometry.x = Math.round(x / vueItem.view.scale) -
					tr.x - ((state != null) ? state.origin.x : 0);
				label.geometry.y = Math.round(y / vueItem.view.scale) -
					tr.y - ((state != null) ? state.origin.y : 0);
				label.style += 'autosize=1;'
			}

			vueItem.getModel().beginUpdate();
			try
			{
                vueItem.addCells([label], (state != null) ? state.cell : null);
				fireEvent(new mxEventObject('textInserted', 'cells', [label]));
				
		    	// Updates size of text after possible change of style via event
				vueItem.autoSizeCell(label);
			}
			finally
			{
				vueItem.getModel().endUpdate();
			}
