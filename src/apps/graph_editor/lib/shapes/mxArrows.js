...
mxShapeArrows2Arrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dy =
    h * 0.5 * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'dy', this.dy))));
  var dx = Math.max(0, Math.min(w, parseFloat(mxUtils.getValue(this.style, 'dx', this.dx))));
  var notch = Math.max(
    0,
    Math.min(w, parseFloat(mxUtils.getValue(this.style, 'notch', this.notch))),
  );
...
mxShapeArrows2Arrow.prototype.getLabelBounds = function (rect) {
  if (mxUtils.getValue(this.style, 'boundedLbl', false)) {
    ...
    if (mxUtils.getValue(this.style, 'flipH', false)) {
      if (direction == mxConstants.DIRECTION_WEST) direction = mxConstants.DIRECTION_EAST;
      else if (direction == mxConstants.DIRECTION_EAST) direction = mxConstants.DIRECTION_WEST;
    }

    if (mxUtils.getValue(this.style, 'flipV', false)) {
      if (direction == mxConstants.DIRECTION_NORTH) direction = mxConstants.DIRECTION_SOUTH;
      else if (direction == mxConstants.DIRECTION_SOUTH) direction = mxConstants.DIRECTION_NORTH;
    }

    if (direction == mxConstants.DIRECTION_NORTH || direction == mxConstants.DIRECTION_SOUTH) {
      dy =
        w * 0.5 * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'dy', this.dy))));
...
Graph.handleFactory[mxShapeArrows2Arrow.prototype.cst.ARROW] = function (state) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        var dx = Math.max(
          0,
          Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))),
        );
        var dy = Math.max(
          0,
          Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))),
        );
          ...
          Math.round(
            100 *
              Math.max(
                ...
                Math.min(
                  bounds.width -
                    parseFloat(mxUtils.getValue(this.state.style, 'notch', this.notch)),
                  bounds.x + bounds.width - pt.x,
                ),
              ),
          ) / 100;
        this.state.style['dy'] =
          Math.round(100 * Math.max(0, Math.min(1, ((pt.y - bounds.y) / bounds.height) * 2))) / 100;
      },
    ),
  ];
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      var notch = Math.max(
        0,
        Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'notch', this.notch))),
      );

      return new mxPoint(bounds.x + notch, bounds.y + bounds.height / 2);
    },
    function (bounds, pt) {
      this.state.style['notch'] =
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx)),
                pt.x - bounds.x,
              ),
            ),
        ) / 100;
...
mxCellRenderer.registerShape(
  mxShapeArrows2TwoWayArrow.prototype.cst.TWO_WAY_ARROW,
  mxShapeArrows2TwoWayArrow,
);
  ...
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        var dx = Math.max(
          0,
          Math.min(bounds.width / 2, parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx))),
        );
        var dy = Math.max(
          0,
          Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))),
      ...
      function (bounds, pt) {
        this.state.style['dx'] =
          Math.round(
            100 * Math.max(0, Math.min(bounds.width / 2, bounds.x + bounds.width - pt.x)),
          ) / 100;
        this.state.style['dy'] =
          Math.round(100 * Math.max(0, Math.min(1, ((pt.y - bounds.y) / bounds.height) * 2))) / 100;
      },
...
mxShapeArrows2StylisedArrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var feather =
    h *
    0.5 *
    Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'feather', this.feather))));
...
mxCellRenderer.registerShape(
  mxShapeArrows2StylisedArrow.prototype.cst.STYLISED_ARROW,
  mxShapeArrows2StylisedArrow,
);
...
Graph.handleFactory[mxShapeArrows2StylisedArrow.prototype.cst.STYLISED_ARROW] = function (state) {
  ...
  var handle3 = Graph.createHandle(
    state,
    ['feather'],
    function (bounds) {
      var feather = Math.max(
        0,
        Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'feather', this.dy))),
      );

      return new mxPoint(bounds.x, bounds.y + (feather * bounds.height) / 2);
    },
    function (bounds, pt) {
      this.state.style['feather'] =
        Math.round(100 * Math.max(0, Math.min(1, ((pt.y - bounds.y) / bounds.height) * 2))) / 100;
    },
  );
...
mxShapeArrows2SharpArrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dy1 =
    h * 0.5 * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'dy1', this.dy1))));
  ...
  var dy1a =
    h * 0.5 * Math.max(0, Math.min(h, parseFloat(mxUtils.getValue(this.style, 'dy1', this.dy1))));
  var x2 = 0;
...
mxCellRenderer.registerShape(
  mxShapeArrows2SharpArrow.prototype.cst.SHARP_ARROW,
...
Graph.handleFactory[mxShapeArrows2SharpArrow.prototype.cst.SHARP_ARROW] = function (state) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        var dx1 = Math.max(
          0,
          Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'dx1', this.dx1))),
        );
        var dy1 = Math.max(
          0,
          Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1))),
        );
        ...
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'dx1', this.dx1)),
                pt.x - bounds.x,
              ),
            ),
        ) / 100;
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds) {
      var dx2 = Math.max(
        0,
        Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'dx2', this.dx2))),
      );

      return new mxPoint(bounds.x + bounds.width - dx2, bounds.y);
    },
    function (bounds, pt) {
      this.state.style['dx2'] =
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'notch', this.notch)),
                bounds.x + bounds.width - pt.x,
              ),
            ),
        ) / 100;
...
mxShapeArrows2SharpArrow2.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dx2 = Math.max(0, Math.min(w, parseFloat(mxUtils.getValue(this.style, 'dx2', this.dx2))));
  var dy3 =
    h * 0.5 * Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'dy3', this.dy3))));
...
mxCellRenderer.registerShape(
  mxShapeArrows2SharpArrow2.prototype.cst.SHARP_ARROW2,
  mxShapeArrows2SharpArrow2,
);
...
Graph.handleFactory[mxShapeArrows2SharpArrow2.prototype.cst.SHARP_ARROW2] = function (state) {
  ...
  var handle4 = Graph.createHandle(
    state,
    ['dx3', 'dy3'],
    function (bounds) {
      var dx3 = Math.max(
        0,
        Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'dx3', this.dx3))),
      );
      var dy3 = Math.max(
        0,
        Math.min(
          1 - parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)) / 2,
          parseFloat(mxUtils.getValue(this.state.style, 'dy3', this.dy3)),
        ),
      );
...
mxShapeArrows2CalloutArrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var arrowHead = Math.max(
    0,
    Math.min(h, parseFloat(mxUtils.getValue(this.style, 'arrowHead', this.arrowHead))),
  );
...
mxCellRenderer.registerShape(
  mxShapeArrows2CalloutArrow.prototype.cst.CALLOUT_ARROW,
  mxShapeArrows2CalloutArrow,
);
...
Graph.handleFactory[mxShapeArrows2CalloutArrow.prototype.cst.CALLOUT_ARROW] = function (state) {
    ...
    Graph.createHandle(
      ...
      function (bounds) {
        var arrowHead = Math.max(
          ...
          Math.min(
            bounds.height / 2,
            parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)),
          ),
        );
        var dx = Math.max(
          0,
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds) {
      ...
      var dy = Math.max(
        0,
        Math.min(bounds.height / 2, parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))),
      );
      var arrowHead = Math.max(
        0,
        Math.min(
      ...
      return new mxPoint(
        ...
        bounds.y + bounds.height / 2 - dy - arrowHead,
      );
    },
    function (bounds, pt) {
      this.state.style['arrowHead'] =
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.height / 2 - parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)),
                bounds.y +
                  bounds.height / 2 -
                  parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)) -
                  pt.y,
...
mxShapeArrows2CalloutArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - dx, h * 0.5 - dy - arrowHead),
  );
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0.5), false, null, (notch + w - dx) * 0.5, -dy),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0.5), false, null, (notch + w - dx) * 0.5, dy),
  );
...
mxShapeArrows2BendArrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var notch = Math.max(
    0,
    Math.min(h, parseFloat(mxUtils.getValue(this.style, 'notch', this.notch))),
  );
  var arrowHead = Math.max(
    0,
    Math.min(h, parseFloat(mxUtils.getValue(this.style, 'arrowHead', this.arrowHead))),
...
mxCellRenderer.registerShape(
  mxShapeArrows2BendArrow.prototype.cst.BEND_ARROW,
  ...
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        ...
        var dy = Math.max(
          0,
          Math.min(arrowHead, parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))),
        );
          ...
          Math.round(
            100 *
              Math.max(
                ...
                Math.min(
                  bounds.width -
                    parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)) * 2.2,
                  bounds.x + bounds.width - pt.x,
                ),
              ),
          ) / 100;
          ...
          Math.round(
            100 *
              Math.max(
                0,
                Math.min(
                  parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)) / 2,
                  bounds.y +
                    parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)) /
                      2 -
                    pt.y,
    ...
    function (bounds, pt) {
      this.state.style['arrowHead'] =
        Math.round(
          100 *
            Math.max(
              2 * parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)),
...
mxShapeArrows2BendArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - dx, arrowHead / 2 - dy),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - dx, 0));
  ...
  constr.push(
    new mxConnectionConstraint(
      ...
      null,
      dy * 2,
      (h - arrowHead / 2 - dy) * 0.5 + arrowHead / 2 + dy,
    ),
  );
  ...
  if (rounded == '1') {
    ...
    constr.push(
      new mxConnectionConstraint(
        ...
        null,
        2 * dy + dy * 0.0586,
        arrowHead / 2 + dy + dy * 0.0586,
      ),
    );
  } else {
    constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, arrowHead / 2 - dy));
    constr.push(
      new mxConnectionConstraint(new mxPoint(0, 0), false, null, dy * 2, arrowHead / 2 + dy),
    );
  }
...
mxShapeArrows2BendDoubleArrow.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var arrowHead = Math.max(
    0,
    Math.min(w, parseFloat(mxUtils.getValue(this.style, 'arrowHead', this.arrowHead))),
  );
  var rounded = mxUtils.getValue(this.style, 'rounded', '0');
...
mxCellRenderer.registerShape(
  mxShapeArrows2BendDoubleArrow.prototype.cst.BEND_DOUBLE_ARROW,
  mxShapeArrows2BendDoubleArrow,
);

mxShapeArrows2BendDoubleArrow.prototype.constraints = null;

Graph.handleFactory[mxShapeArrows2BendDoubleArrow.prototype.cst.BEND_DOUBLE_ARROW] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        ...
        var dx = Math.max(
          0,
          Math.min(
            Math.min(bounds.width, bounds.height) - arrowHead,
            parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx)),
          ),
        );
        var dy = Math.max(
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      ...
      var arrowHead = Math.max(
        0,
        Math.min(
          Math.min(bounds.height, bounds.width) - dx,
          parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)),
        ),
      );
...
mxShapeArrows2BendDoubleArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, arrowHead / 2 + dy, h - dx),
  );
        ...
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.width / 2 - parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx)),
...
mxShapeArrows2CalloutDoubleArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx, h * 0.5 - dy - arrowHead),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx, h * 0.5 + dy + arrowHead),
  );
...
mxCellRenderer.registerShape(
  mxShapeArrows2CalloutQuadArrow.prototype.cst.CALLOUT_QUAD_ARROW,
  mxShapeArrows2CalloutQuadArrow,
);

mxShapeArrows2CalloutQuadArrow.prototype.constraints = null;

Graph.handleFactory[mxShapeArrows2CalloutQuadArrow.prototype.cst.CALLOUT_QUAD_ARROW] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        ...
        var dy = Math.max(
          0,
          Math.min(bounds.height, parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy))),
        );
...
mxShapeArrows2CalloutQuadArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(new mxConnectionConstraint(new mxPoint(1, 0.5), false));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + dy, h * 0.5 - notch),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + notch, h * 0.5 - notch),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + notch, h * 0.5 - dy),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + dy, h * 0.5 + notch),
  );
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 - dy - arrowHead, h - dx),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + dy + arrowHead, h - dx),
  );
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 - dy - arrowHead, dx),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 + dy + arrowHead, dx),
  );
...
mxCellRenderer.registerShape(
  mxShapeArrows2CalloutDouble90Arrow.prototype.cst.CALLOUT_DOUBLE_90_ARROW,
    ...
    Graph.createHandle(
      ...
      function (bounds) {
        var arrowHead = Math.max(
          0,
          Math.min(
            parseFloat(mxUtils.getValue(this.state.style, 'dy2', this.dy2)) / 2,
            parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)),
          ),
        );
        var dx1 = Math.max(
          0,
          Math.min(
            bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'dx2', this.dx2)),
            parseFloat(mxUtils.getValue(this.state.style, 'dx1', this.dx1)),
          ),
        );
        var dy1 = Math.max(
          0,
          Math.min(
            parseFloat(mxUtils.getValue(this.state.style, 'dy2', this.dy2)) / 2 - arrowHead,
            parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)),
          ),
        );

        return new mxPoint(
          bounds.x + bounds.width - dx1,
          bounds.y + parseFloat(mxUtils.getValue(this.state.style, 'dy2', this.dy2)) / 2 - dy1,
        );
      },
      function (bounds, pt) {
        this.state.style['dx1'] =
          Math.round(
            100 *
              Math.max(
                ...
                Math.min(
                  bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'dx2', this.dx2)),
                  bounds.height - parseFloat(mxUtils.getValue(this.state.style, 'dy2', this.dy2)),
                  bounds.x + bounds.width - pt.x,
                ),
              ),
          ) / 100;
    ...
    function (bounds) {
      ...
      var dy2 = Math.max(
        parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)) +
          parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)),
        Math.min(
          bounds.height - parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)),
          ...
          100 *
            Math.max(
              parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)) +
                parseFloat(mxUtils.getValue(this.state.style, 'arrowHead', this.arrowHead)),
              Math.min(
                bounds.height - parseFloat(mxUtils.getValue(this.state.style, 'dx1', this.dx1)),
                pt.y - bounds.y,
              ),
            ),
        ) / 100;
...
mxShapeArrows2CalloutDouble90Arrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx2 / 2 + dy1 + arrowHead, h - dx1),
  );
...
mxCellRenderer.registerShape(
  mxShapeArrows2QuadArrow.prototype.cst.QUAD_ARROW,
  mxShapeArrows2QuadArrow,
);
...
mxCellRenderer.registerShape(
  mxShapeArrows2TriadArrow.prototype.cst.TRIAD_ARROW,
  mxShapeArrows2TriadArrow,
);
        ...
        Math.round(
          100 *
            Math.max(
              2 * parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)),
              Math.min(
                bounds.height - parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx)),
                bounds.width - parseFloat(mxUtils.getValue(this.state.style, 'dx', this.dx)) * 2,
                bounds.y + bounds.height - pt.y,
              ),
            ),
        ) / 100;
...
mxShapeArrows2TriadArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(
      ...
      false,
      null,
      w * 0.5 - arrowHead * 0.5 + dy,
      (dx + h - arrowHead + dy) * 0.5,
    ),
  );
  constr.push(
    new mxConnectionConstraint(
...
mxCellRenderer.registerShape(
  mxShapeArrows2TailedArrow.prototype.cst.TAILED_ARROW,
  mxShapeArrows2TailedArrow,
);
...
Graph.handleFactory[mxShapeArrows2TailedArrow.prototype.cst.TAILED_ARROW] = function (state) {
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds) {
      ...
      return new mxPoint(
        bounds.x + bounds.width - dx1,
        bounds.y + bounds.height / 2 - dy1 - arrowHead,
      );
    },
    function (bounds, pt) {
      this.state.style['arrowHead'] =
        Math.round(
          100 *
            Math.max(
              0,
              Math.min(
                bounds.height / 2 - parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)),
                bounds.y + bounds.height / 2 - pt.y,
              ),
            ),
        ) / 100;
      ...
      this.state.style['dy2'] =
        Math.round(
          100 *
            Math.max(
              parseFloat(mxUtils.getValue(this.state.style, 'dy1', this.dy1)),
              Math.min(bounds.height / 2, bounds.y + bounds.height / 2 - pt.y),
            ),
        ) / 100;
    },
  );
...
mxShapeArrows2TailedArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, (w - dx1 + x2) * 0.5, h * 0.5 - dy1),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - dx1, h * 0.5 - dy1 - arrowHead),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, h * 0.5 + dy2));
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx2, h * 0.5 + dy2));
  constr.push(
...
mxCellRenderer.registerShape(
  mxShapeArrows2TailedNotchedArrow.prototype.cst.TAILED_NOTCHED_ARROW,
  mxShapeArrows2TailedNotchedArrow,
);
...
mxCellRenderer.registerShape(
  mxShapeArrows2StripedArrow.prototype.cst.STRIPED_ARROW,
  mxShapeArrows2StripedArrow,
);
...
mxCellRenderer.registerShape(
  mxShapeArrows2JumpInArrow.prototype.cst.JUMP_IN_ARROW,
...
Graph.handleFactory[mxShapeArrows2JumpInArrow.prototype.cst.JUMP_IN_ARROW] = function (state) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dx'] =
          Math.round(100 * Math.max(0, Math.min(bounds.width, bounds.x + bounds.width - pt.x))) /
          100;
        this.state.style['dy'] =
          Math.round(
            100 *
...
mxCellRenderer.registerShape(
  mxShapeArrows2UTurnArrow.prototype.cst.U_TURN_ARROW,
  mxShapeArrows2UTurnArrow,
);
...
Graph.handleFactory[mxShapeArrows2UTurnArrow.prototype.cst.U_TURN_ARROW] = function (state) {
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds, pt) {
      ...
      var dx = (bounds.height - arrowHead / 2 + dy) / 2;
      this.state.style['dx2'] =
        Math.round(100 * Math.max(0, Math.min(Math.max(bounds.width, dx), pt.x - bounds.x - dx))) /
        100;
    },
  );
      ...
      this.state.style['arrowHead'] =
        Math.round(
          100 *
            Math.max(
              2 * parseFloat(mxUtils.getValue(this.state.style, 'dy', this.dy)),
              Math.min(bounds.height / 2, pt.y - bounds.y),
            ),
        ) / 100;
    },
  );
...
mxShapeArrows2UTurnArrow.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx, 0));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx + dx2, arrowHead * 0.5),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx, arrowHead));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, (dx + w) * 0.5, h - 2 * dy),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, Math.max(w, dx), h - 2 * dy),
  );
  ...
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, dx, h));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, (h + arrowHead * 0.5 - dy) * 0.5),
  );
  constr.push(
    new mxConnectionConstraint(
      new mxPoint(0, 0),
