...
mxCellRenderer.registerShape(
  mxShapeInfographicNumEntryVert.prototype.cst.NUM_ENTRY_VERT,
  mxShapeInfographicNumEntryVert,
);

Graph.handleFactory[mxShapeInfographicNumEntryVert.prototype.cst.NUM_ENTRY_VERT] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dy'] =
          Math.round(100 * Math.max(0, Math.min(bounds.height, bounds.width, pt.y - bounds.y))) /
          100;
      },
    ),
  ];
...
mxShapeInfographicBendingArch.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var startAngle =
    ...
    Math.max(
      0,
      Math.min(1, parseFloat(mxUtils.getValue(this.style, 'startAngle', this.startAngle))),
    );
  var endAngle =
    2 *
    Math.PI *
    Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'endAngle', this.endAngle))));
  var arcWidth =
    1 -
    Math.max(0, Math.min(1, parseFloat(mxUtils.getValue(this.style, 'arcWidth', this.arcWidth))));
  var rx = w * 0.5;
  var ry = h * 0.5;
  var rx2 = rx * arcWidth;
  var ry2 = ry * arcWidth;
...
mxCellRenderer.registerShape(
  mxShapeInfographicBendingArch.prototype.cst.BENDING_ARCH,
  mxShapeInfographicBendingArch,
);
...
Graph.handleFactory[mxShapeInfographicBendingArch.prototype.cst.BENDING_ARCH] = function (state) {
      ...
      function (bounds) {
        var startAngle =
          ...
          Math.max(
            ...
            Math.min(
              1,
              parseFloat(mxUtils.getValue(this.state.style, 'startAngle', this.startAngle)),
            ),
          );

        return new mxPoint(
          bounds.x + bounds.width * 0.5 + Math.sin(startAngle) * bounds.width * 0.5,
          bounds.y + bounds.height * 0.5 - Math.cos(startAngle) * bounds.height * 0.5,
        );
      },
      function (bounds, pt) {
        var handleX =
          Math.round(
            100 *
              Math.max(
                -1,
                Math.min(1, (pt.x - bounds.x - bounds.width * 0.5) / (bounds.width * 0.5)),
              ),
          ) / 100;
        var handleY =
          -Math.round(
            100 *
              Math.max(
                -1,
                Math.min(1, (pt.y - bounds.y - bounds.height * 0.5) / (bounds.height * 0.5)),
              ),
          ) / 100;
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      var endAngle =
        ...
        Math.max(
          0,
          Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'endAngle', this.endAngle))),
        );

      return new mxPoint(
        bounds.x + bounds.width * 0.5 + Math.sin(endAngle) * bounds.width * 0.5,
        bounds.y + bounds.height * 0.5 - Math.cos(endAngle) * bounds.height * 0.5,
      );
    },
    function (bounds, pt) {
      var handleX =
  ...
  var handle3 = Graph.createHandle(
    ...
    ['arcWidth'],
    function (bounds) {
      var arcWidth = Math.max(
        0,
        Math.min(1, parseFloat(mxUtils.getValue(this.state.style, 'arcWidth', this.arcWidth))),
      );

      return new mxPoint(bounds.x + bounds.width / 2, bounds.y + arcWidth * bounds.height * 0.5);
    },
    function (bounds, pt) {
      this.state.style['arcWidth'] =
        Math.round(
          100 *
            Math.max(
              0,
...
mxCellRenderer.registerShape(
  mxShapeInfographicParallelogram.prototype.cst.PARALLELOGRAM,
  mxShapeInfographicParallelogram,
);

Graph.handleFactory[mxShapeInfographicParallelogram.prototype.cst.PARALLELOGRAM] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dx'] =
          Math.round(100 * Math.max(0, Math.min(bounds.width / 2, pt.x - bounds.x))) / 100;
      },
    ),
  ];
...
mxCellRenderer.registerShape(
  mxShapeInfographicRibbonRolled.prototype.cst.RIBBON_ROLLED,
...
Graph.handleFactory[mxShapeInfographicRibbonRolled.prototype.cst.RIBBON_ROLLED] = function (state) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dx'] =
          Math.round(100 * Math.max(0, Math.min(bounds.width, pt.x - bounds.x))) / 100;
        this.state.style['dy'] =
          Math.round(100 * Math.max(0, Math.min(bounds.height / 2, pt.y - bounds.y))) / 100;
      },
    ),
  ];
...
mxCellRenderer.registerShape(
  mxShapeInfographicRibbonDoubleFolded.prototype.cst.RIBBON_DOUBLE_FOLDED,
  mxShapeInfographicRibbonDoubleFolded,
);
...
mxShapeInfographicRibbonFrontFolded.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var notch = Math.max(
    0,
    Math.min(w - dx, parseFloat(mxUtils.getValue(this.style, 'notch', this.notch))),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicRibbonFrontFolded.prototype.cst.RIBBON_FRONT_FOLDED,
  mxShapeInfographicRibbonFrontFolded,
);
...
Graph.handleFactory[
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      ...
      return new mxPoint(
        bounds.x + bounds.width - notch,
        bounds.y + dy + (bounds.height - dy) * 0.5,
      );
    },
    function (bounds, pt) {
      this.state.style['notch'] =
        Math.round(100 * Math.max(0, Math.min(bounds.width, bounds.width + bounds.x - pt.x))) / 100;
    },
  );
...
mxShapeInfographicRibbonFrontFolded.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(new mxConnectionConstraint(new mxPoint(1, 0), false, null, 0, dy));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - notch, (h + dy) * 0.5),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(1, 1), false));
...
mxCellRenderer.registerShape(
  mxShapeInfographicRibbonBackFolded.prototype.cst.RIBBON_BACK_FOLDED,
...
mxCellRenderer.registerShape(
  mxShapeInfographicBanner.prototype.cst.BANNER,
  mxShapeInfographicBanner,
);
...
Graph.handleFactory[mxShapeInfographicBanner.prototype.cst.BANNER] = function (state) {
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      ...
      var notch = Math.max(
        0,
        Math.min(dx, parseFloat(mxUtils.getValue(this.state.style, 'notch', this.notch))),
      );
...
mxCellRenderer.registerShape(
  mxShapeInfographicCircularCallout.prototype.cst.CIRCULAR_CALLOUT,
  mxShapeInfographicCircularCallout,
);

Graph.handleFactory[mxShapeInfographicCircularCallout.prototype.cst.CIRCULAR_CALLOUT] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
...
mxCellRenderer.registerShape(
  mxShapeInfographicShadedTriangle.prototype.cst.SHADED_TRIANGLE,
  mxShapeInfographicShadedTriangle,
);
...
mxCellRenderer.registerShape(
  mxShapeInfographicShadedPyramid.prototype.cst.SHADED_PYRAMID,
  mxShapeInfographicShadedPyramid,
);
...
mxCellRenderer.registerShape(
  mxShapeInfographicPyramidStep.prototype.cst.PYRAMID_STEP,
  mxShapeInfographicPyramidStep,
);
...
mxCellRenderer.registerShape(
  mxShapeInfographicCylinder.prototype.cst.CYLINDER,
  mxShapeInfographicCylinder,
);
...
mxShapeInfographicCylinder.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, dy * 0.5 + (h - dy) * 0.25),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, dy * 0.5 + (h - dy) * 0.5),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, dy * 0.5 + (h - dy) * 0.75),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, h - dy * 0.5));
  constr.push(new mxConnectionConstraint(new mxPoint(0.5, 1), false));
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, h - dy * 0.5));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, dy * 0.5 + (h - dy) * 0.75),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, dy * 0.5 + (h - dy) * 0.5),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, dy * 0.5 + (h - dy) * 0.25),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicCircularCallout2.prototype.cst.CIRCULAR_CALLOUT_2,
...
mxShapeInfographicBannerSingleFold.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dx2 = Math.max(
    0,
    Math.min(w - dx - 2 * dy, parseFloat(mxUtils.getValue(this.style, 'dx2', this.dx2))),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicBannerSingleFold.prototype.cst.BANNER_SINGLE_FOLD,
  mxShapeInfographicBannerSingleFold,
);
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds, pt) {
      ...
      this.state.style['dx2'] =
        Math.round(100 * Math.max(0, Math.min(bounds.width - dx - 2 * dy, pt.x - bounds.x))) / 100;
    },
  );
...
mxShapeInfographicBannerSingleFold.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - (dx + 2 * dy) * 0.75, h),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - (dx + 2 * dy) * 0.5, h),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w - (dx + 2 * dy) * 0.25, h),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicShadedCube.prototype.cst.SHADED_CUBE,
  mxShapeInfographicShadedCube,
);
    ...
    Graph.createHandle(
      ...
      function (bounds) {
        var isoAngle =
          (Math.max(
            0.01,
            Math.min(94, parseFloat(mxUtils.getValue(this.state.style, 'isoAngle', this.isoAngle))),
          ) *
            Math.PI) /
          200;
      ...
      function (bounds, pt) {
        this.state.style['isoAngle'] =
          Math.round(100 * Math.max(0, Math.min(100, pt.y - bounds.y))) / 100;
      },
    ),
  ];
...
mxShapeInfographicShadedCube.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, isoH + (h - 2 * isoH) * 0.25),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, isoH + (h - 2 * isoH) * 0.5),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w, isoH + (h - 2 * isoH) * 0.75),
  );
  ...
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, h - isoH));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, isoH + (h - 2 * isoH) * 0.75),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, isoH + (h - 2 * isoH) * 0.5),
  );
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, 0, isoH + (h - 2 * isoH) * 0.25),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicPartConcEllipse.prototype.cst.PART_CONC_ELLIPSE,
  mxShapeInfographicPartConcEllipse,
);

Graph.handleFactory[mxShapeInfographicPartConcEllipse.prototype.cst.PART_CONC_ELLIPSE] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
...
mxShapeInfographicBannerHalfFold.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dx2 = Math.max(
    0,
    Math.min(w - dx, parseFloat(mxUtils.getValue(this.style, 'dx2', this.dx2))),
  );
  var notch = Math.max(
    0,
    Math.min(h - dx, parseFloat(mxUtils.getValue(this.style, 'notch', this.notch))),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicBannerHalfFold.prototype.cst.BANNER_HALF_FOLD,
  mxShapeInfographicBannerHalfFold,
);

Graph.handleFactory[mxShapeInfographicBannerHalfFold.prototype.cst.BANNER_HALF_FOLD] = function (
  state,
) {
  var handles = [
    Graph.createHandle(
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds, pt) {
      this.state.style['notch'] =
        Math.round(100 * Math.max(0, Math.min(bounds.height, bounds.height + bounds.y - pt.y))) /
        100;
    },
  );
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds, pt) {
      ...
      this.state.style['dx2'] =
        Math.round(100 * Math.max(0, Math.min(bounds.width - dx, pt.x - bounds.x))) / 100;
    },
  );
...
mxShapeInfographicCircularDial.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var dy = Math.max(
    0,
    Math.min(h * 0.5 - 10, w * 0.5, parseFloat(mxUtils.getValue(this.style, 'dy', this.dy))),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicCircularDial.prototype.cst.CIRCULAR_DIAL,
  ...
  var handles = [
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dy'] =
          Math.round(
            100 * Math.max(0, Math.min(bounds.height / 2, bounds.width / 2, pt.y - bounds.y)),
          ) / 100;
      },
    ),
  ];
...
mxShapeInfographicCircularDial.prototype.getConstraints = function (style, w, h) {
  ...
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 - rx, (rx + h) * 0.5),
  );
  constr.push(new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 - rx, rx));
  constr.push(
    new mxConnectionConstraint(new mxPoint(0, 0), false, null, w * 0.5 - rx * 0.71, rx * 0.29),
  );
...
mxShapeInfographicRibbonSimple.prototype.paintVertexShape = function (c, x, y, w, h) {
  ...
  var notch1 = Math.max(
    0,
    Math.min(w, parseFloat(mxUtils.getValue(this.style, 'notch1', this.notch2))),
  );
  var notch2 = Math.max(
    0,
    Math.min(w, parseFloat(mxUtils.getValue(this.style, 'notch2', this.notch2))),
  );
...
mxCellRenderer.registerShape(
  mxShapeInfographicRibbonSimple.prototype.cst.RIBBON_SIMPLE,
  mxShapeInfographicRibbonSimple,
);
    ...
    Graph.createHandle(
      ...
      function (bounds) {
        var notch1 = Math.max(
          ...
          Math.min(
            bounds.width,
            parseFloat(mxUtils.getValue(this.state.style, 'notch1', this.notch1)),
          ),
        );
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      var notch2 = Math.max(
        ...
        Math.min(
          bounds.width,
          parseFloat(mxUtils.getValue(this.state.style, 'notch2', this.notch2)),
        ),
      );
...
mxCellRenderer.registerShape(
  mxShapeInfographicBarCallout.prototype.cst.BAR_CALLOUT,
  mxShapeInfographicBarCallout,
);
    ...
    Graph.createHandle(
      ...
      function (bounds, pt) {
        this.state.style['dx'] =
          Math.round(100 * Math.max(0, Math.min(bounds.width, pt.x - bounds.x))) / 100;
        this.state.style['dy'] =
          Math.round(100 * Math.max(0, Math.min(bounds.height, bounds.y + bounds.height - pt.y))) /
          100;
      },
    ),
  ];
