...
// TEN9: Add spectrum-colorpicker import here for webpack to pickup and handle
...
export default function luckysheetHandler() {
    ...
    $("#luckysheet-cell-main, #luckysheetTableContent").mousedown(function (event) {
        ...
        if (event.which == "3") {
            ...
            // TEN9 : check if it's single cell selection or not
            if (obj_s["row"]) {
                if (obj_s["row"][0] !== obj_s["row"][1]) {
                    $("#luckysheet-cols-rows-shift").show();
                }
            }
            ...
            // TEN9 : Hide unhide option when all rows and columns selected
    ...
    // TEN9: Comment out the core drop handling for image inserts. We are handling it at the wrapper level
    //监听拖拽
    ...
    // TEN9: Comment out the core dragover handling for image inserts. We are handling it at the wrapper level
    ...
    $("#luckysheet-left-top").on("contextmenu", function (e) {
        ...
        if (e.which == 3) {
            ...
            $("#luckysheet-cols-rows-add").css('display','block');
            $("#luckysheet-delete-text").css('display','block');
            showrightclickmenu($("#luckysheet-rightclick-menu"), x, y);
        }
        e.preventDefault();
