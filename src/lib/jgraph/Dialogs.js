...
// TEN9: Custom function to save xml
var ExportDialogXml = function(editorUi)
{
	var graph = editorUi.editor.graph;
	var bounds = graph.getGraphBounds();
	var scale = graph.view.scale;
	...
	var saveBtn = mxUtils.button(mxResources.get('export'), mxUtils.bind(this, function()
	{
		...
		else
		{
			...
			ExportDialog.lastBorderValue = b;
			ExportDialog.exportXmlFile(editorUi, name, format, bg, s, b, dpi);
		}
	}));
	saveBtn.className = 'geBtn gePrimaryBtn';
...
ExportDialog.exportXmlFile = function(editorUi, name, format, bg, s, b, dpi)
{
	var graph = editorUi.editor.graph;
	
	if (format == 'xml')
	{
    	ExportDialog.saveLocalFileXml(editorUi, mxUtils.getXml(editorUi.editor.getGraphXml()), name, format);
	}
	else
	{
...
ExportDialog.saveLocalFileXml = function(editorUi, data, filename, format)
{
	if (data.length < MAX_REQUEST_SIZE)
	{
		editorUi.hideDialog();
		...
		var fileName = "fileName1"+'_RQ.xml';
		var fileType = '.xml';
