...
Graph.handleFactory[mxShapeLeanTimeline.prototype.cst.SHAPE_TIMELINE] = function (state) {
  var handles = [
    Graph.createHandle(
      ...
      function (bounds) {
        var dy1 = Math.max(
          0,
          Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy1', 0))),
        );
  ...
  var handle2 = Graph.createHandle(
    ...
    function (bounds) {
      var dx2 = Math.max(
        0,
        Math.min(
          parseFloat(mxUtils.getValue(this.state.style, 'dx3', 0)),
          parseFloat(mxUtils.getValue(this.state.style, 'dx2', 0)),
        ),
      );
      var dy2 = Math.max(
        0,
        Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy2', 0))),
      );
      ...
      this.state.style['dx2'] =
        Math.round(
          1000 *
            Math.max(
              0,
              Math.min(parseFloat(mxUtils.getValue(this.state.style, 'dx3', 0)), pt.x - bounds.x),
            ),
        ) / 1000;
  ...
  var handle3 = Graph.createHandle(
    ...
    function (bounds) {
      var dx3 = Math.max(
        parseFloat(mxUtils.getValue(this.state.style, 'dx2', 0)),
        Math.min(
          parseFloat(mxUtils.getValue(this.state.style, 'dx4', 0)),
          parseFloat(mxUtils.getValue(this.state.style, 'dx3', 0)),
        ),
      );
      var dy3 = Math.max(
        0,
        Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy3', 0))),
      );
      ...
      this.state.style['dx3'] =
        Math.round(
          1000 *
            Math.max(
              parseFloat(mxUtils.getValue(this.state.style, 'dx2', 0)),
              Math.min(parseFloat(mxUtils.getValue(this.state.style, 'dx4', 0)), pt.x - bounds.x),
            ),
        ) / 1000;
  ...
  var handle4 = Graph.createHandle(
    ...
    function (bounds) {
      var dx4 = Math.max(
        parseFloat(mxUtils.getValue(this.state.style, 'dx3', 0)),
        Math.min(
          parseFloat(mxUtils.getValue(this.state.style, 'dx5', 0)),
          parseFloat(mxUtils.getValue(this.state.style, 'dx4', 0)),
        ),
      );
      var dy4 = Math.max(
        0,
        Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy4', 0))),
      );

      return new mxPoint(bounds.x + dx4, bounds.y + dy4 * bounds.height);
    },
    function (bounds, pt) {
      this.state.style['dx4'] =
        Math.round(
          1000 *
            Math.max(
              parseFloat(mxUtils.getValue(this.state.style, 'dx3', 0)),
              Math.min(parseFloat(mxUtils.getValue(this.state.style, 'dx5', 0)), pt.x - bounds.x),
            ),
        ) / 1000;
      this.state.style['dy4'] =
        Math.round(1000 * Math.max(0, Math.min(1, (pt.y - bounds.y) / bounds.height))) / 1000;
    },
  );
  ...
  var handle5 = Graph.createHandle(
    state,
    ['dx5', 'dy5'],
    function (bounds) {
      var dx5 = Math.max(
        parseFloat(mxUtils.getValue(this.state.style, 'dx4', 0)),
        Math.min(bounds.width, parseFloat(mxUtils.getValue(this.state.style, 'dx5', 0))),
      );
      var dy5 = Math.max(
        0,
        Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy5', 0))),
      );

      return new mxPoint(bounds.x + dx5, bounds.y + dy5 * bounds.height);
    },
    function (bounds, pt) {
      this.state.style['dx5'] =
        Math.round(
          1000 *
            Math.max(
              parseFloat(mxUtils.getValue(this.state.style, 'dx4', 0)),
  ...
  var handle6 = Graph.createHandle(
    state,
    ['dy6'],
    function (bounds) {
      var dy6 = Math.max(
        0,
        Math.min(100, parseFloat(mxUtils.getValue(this.state.style, 'dy6', 0))),
      );

      return new mxPoint(bounds.x + bounds.width, bounds.y + dy6 * bounds.height);
    },
    function (bounds, pt) {
      this.state.style['dy6'] =
        Math.round(1000 * Math.max(0, Math.min(1, (pt.y - bounds.y) / bounds.height))) / 1000;
    },
  );
