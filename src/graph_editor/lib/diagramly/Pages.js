...
SelectPage.prototype.execute = function () {
  ...
  if (this.page != null && prevIndex >= 0) {
    ...
    graph.gridEnabled =
      graph.gridEnabled && (!this.ui.editor.isChromelessView() || urlParams['grid'] == '1');
  ...
  if (!this.editor.graph.standalone) {
    ...
    var updateTabs = mxUtils.bind(this, function () {
      ...
      if (p != null && p != lastPage) {
        if (p.viewState == null || p.viewState.scrollLeft == null) {
          ...
          graph.container.scrollLeft =
            graph.view.translate.x * graph.view.scale + p.viewState.scrollLeft;
          graph.container.scrollTop =
            graph.view.translate.y * graph.view.scale + p.viewState.scrollTop;
        }
      ...
      if (typeof MathJax !== 'undefined' && typeof MathJax.Hub !== 'undefined') {
        ...
        typeof Editor.MathJaxClear !== 'undefined' &&
        (this.editor == null || !this.editor.graph.mathEnabled)
      ) {
    ...
    this.editor.graph.model.addListener(
      ...
      mxUtils.bind(this, function (sender, evt) {
        ...
        for (var i = 0; i < changes.length; i++) {
          if (
            changes[i] instanceof SelectPage ||
...
EditorUi.prototype.updateTabContainer = function () {
  ...
  if (this.tabContainer != null && this.pages != null) {
    ...
    var tabWidth = Math.min(
      140,
      Math.max(20, (this.tabContainer.clientWidth - btnWidth) / this.pages.length) + 1,
    );
    var startIndex = null;
    ...
    for (var i = 0; i < this.pages.length; i++) {
      ...
      mxUtils.bind(this, function (index, tab) {
        ...
        i,
        this.createTabForPage(
          this.pages[i],
          tabWidth,
          this.pages[i] != this.getCurrentPage(),
          i + 1,
        ),
      );
    }
    ...
    if (wrapper.clientWidth > this.tabContainer.clientWidth - btnWidth) {
      ...
      mxEvent.addListener(
        ...
        mxUtils.bind(this, function (evt) {
          ...
          mxUtils.setOpacity(
            temp2,
            wrapper.scrollLeft < wrapper.scrollWidth - wrapper.clientWidth ? 100 : fade,
          );
          mxEvent.consume(evt);
        }),
      );
...
EditorUi.prototype.createPageMenuTab = function () {
  var tab = this.createControlTab(
    3,
    '<div class="geSprite geSprite-dots" style="display:inline-block;margin-top:5px;width:21px;height:21px;"></div>',
  );
        ...
        mxUtils.bind(this, function (menu, parent) {
          for (var i = 0; i < this.pages.length; i++) {
            mxUtils.bind(this, function (index) {
              ...
              //if (this.pages[index] == this.getCurrentPage())
              if (
                this.pages[index] == this.getCurrentPage() ||
                this.getCurrentPage() == undefined
              ) {
...
EditorUi.prototype.createPageInsertTab = function () {
  var tab = this.createControlTab(
    4,
    '<div class="geSprite geSprite-plus" style="display:inline-block;width:21px;height:21px;"></div>',
  );
  ...
  mxEvent.addGestureListeners(
    ...
    mxUtils.bind(this, function (evt) {
      if (
        graph.isEnabled() &&
        !graph.isMouseDown &&
        ((mxEvent.isTouchEvent(evt) && pageWasActive) || mxEvent.isPopupTrigger(evt))
      ) {
...
(function () {
  var codec = new mxObjectCodec(new ChangePage(), [
    'ui',
    'relatedPage',
    'index',
    'neverShown',
