...
DrawioFile.prototype.mergeFile = function (file, success, error, diffShadow) {
  ...
  try {
    ...
    var shadow =
      this.shadowPages != null
        ? this.shadowPages
        : this.ui.getPagesForNode(mxUtils.parseXml(this.shadowData).documentElement);

    // Loads new document as shadow document
    var pages = this.ui.getPagesForNode(mxUtils.parseXml(file.data).documentElement);

    if (pages != null && pages.length > 0) {
      this.shadowPages = pages;

      // Creates a patch for backup if the checksum fails
      this.backupPatch = this.isModified() ? this.ui.diffPages(shadow, this.ui.pages) : null;

      // Patches the current document
      var patches = [this.ui.diffPages(diffShadow != null ? diffShadow : shadow, this.shadowPages)];
    ...
    if (this.errorReportsEnabled) {
      ...
      var fn = mxUtils.bind(this, function (file) {
          ...
          file != null
            ? this.compressReportData(
                this.getAnonymizedXmlForPages(
                  this.ui.getPagesForNode(mxUtils.parseXml(file.data).documentElement),
                ),
                25000,
              )
            : 'n/a';
        ...
        this.sendErrorReport(
          'Checksum Error in ' + functionName + ' ' + this.getHash(),
      ...
      EditorUi.logError(
        'Checksum Error in ' + functionName + ' ' + this.getId(),
        null,
        this.getMode() + '.' + this.getId(),
        'user_' + uid + (this.sync != null ? '-client_' + this.sync.clientId : '-nosync'),
      );
      ...
      try {
        EditorUi.logEvent({
          ...
          action: functionName,
          label: 'user_' + uid + (this.sync != null ? '-client_' + this.sync.clientId : '-nosync'),
        });
      } catch (e) {
        // ignore
      }
...
DrawioFile.prototype.sendErrorReport = function (title, details, error, max) {
  try {
    var shadow = this.compressReportData(this.getAnonymizedXmlForPages(this.shadowPages), 25000);
    ...
    EditorUi.sendReport(
      title +
        ...
        DrawioFile.SYNC +
        (this.sync != null
          ? (this.sync.enabled ? ' enabled' : '') + (this.sync.isConnected() ? ' connected' : '')
          : '') +
        '\nPlugins=' +
...
DrawioFile.prototype.patch = function (patches, resolver, undoable) {
  ...
  try {
    // Applies patches
    for (var i = 0; i < patches.length; i++) {
      this.ui.pages = this.ui.patchPages(
        this.ui.pages,
        patches[i],
        true,
        resolver,
...
DrawioFile.prototype.updateFileData = function () {
  this.setData(
    this.ui.getFileData(null, null, null, null, null, null, null, null, this, !this.isCompressed()),
  );
};
...
DrawioFile.prototype.addAllSavedStatus = function (status) {
  if (this.ui.statusContainer != null && this.ui.getCurrentFile() == this) {
    status =
      status != null ? status : mxUtils.htmlEntities(mxResources.get(this.allChangesSavedKey));
...
DrawioFile.prototype.addUnsavedStatus = function (err) {
  if (
    ...
    this.ui.statusContainer != null &&
    this.ui.getCurrentFile() == this
  ) {
...
DrawioFile.prototype.addConflictStatus = function (fn, message) {
  ...
  this.setConflictStatus(
    mxUtils.htmlEntities(mxResources.get('fileChangedSync')) +
  ...
  var links =
    this.ui.statusContainer != null ? this.ui.statusContainer.getElementsByTagName('div') : null;
...
DrawioFile.prototype.setConflictStatus = function (message) {
  this.ui.editor.setStatus(
    '<div title="' +
      message +
      '" class="geStatusAlert geBlink" style="cursor:pointer;overflow:hidden;">' +
      message +
      ' <a href="https://www.diagrams.net/doc/faq/synchronize" target="_blank"><img border="0" ' +
      'style="margin-left:2px;cursor:help;opacity:0.5;width:16px;height:16px;" valign="bottom" src="' +
      Editor.helpImage +
      '" style=""/></a></div>',
  );
};
...
DrawioFile.prototype.redirectToNewApp = function (error, details) {
  ...
  if (!this.redirectDialogShowing) {
    ...
    var url =
      ...
      '/' +
      this.ui.getSearch(['create', 'title', 'mode', 'url', 'drive', 'splash', 'state']) +
      '#' +
      this.getHash();
    ...
    if (this.isModified()) {
      ...
      if (this.isTrashed()) {
        this.addAllSavedStatus(
          mxUtils.htmlEntities(mxResources.get(this.allChangesSavedKey)) +
            ' (' +
  ...
  var overwrite = mxUtils.bind(this, function () {
    if (this.ui.spinner.spin(document.body, mxResources.get(this.savingSpinnerKey))) {
      ...
      this.save(
        ...
        null,
        true,
        isRepoFile && err != null ? err.commitMessage : null,
      );
    }
  });
...
DrawioFile.prototype.sendFileChanges = function () {
  try {
    if (this.p2pCollab != null && this.sync != null) {
      ...
      this.sync.sendFileChanges(
        this.ui.getPagesForNode(mxUtils.parseXml(this.getData()).documentElement),
        this.desc,
      );
...
DrawioFile.prototype.fileSaved = function (savedData, lastDesc, success, error, token) {
  ...
  try {
    ...
    if (this.sync == null || this.isOptimisticSync()) {
      ...
      this.sync.fileSaved(
        this.ui.getPagesForNode(mxUtils.parseXml(savedData).documentElement),
        lastDesc,
        success,
        error,
        token,
...
DrawioFile.prototype.isAutosaveRevision = function () {
  ...
  return (
    this.lastAutosaveRevision == null ||
    now - this.lastAutosaveRevision > this.maxAutosaveRevisionDelay
  );
};
